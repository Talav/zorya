...
func isEmptyValue(v reflect.Value) bool {
	//nolint:exhaustive // Only handling types that can be empty - matches encoding/json behavior
	switch v.Kind() {
...
func isNilValue(v any) bool {
	...
	//nolint:exhaustive // Only checking types that can be nil
	switch vv.Kind() {
...
//
// Typically you will use a registry and `Registry.Schema()` to generate
// schemas for your types.
//
...
func (s *Schema) MarshalJSON() ([]byte, error) {
	// Convert to OpenAPI 3.1 format: nullable types use type array ["type", "null"]
	typ := s.determineType()

	// Use explicit ContentMediaType if set, otherwise fall back to computed value
	contentMediaType := s.ContentMediaType
	if contentMediaType == "" && s.Format == formatBinary {
		contentMediaType = "application/octet-stream"
	}

	props := s.Properties
	filtered := make(map[string]*Schema, len(s.Properties))
	hasHidden := false
	...
	fields := make([]jsonFieldInfo, 0, 20)
	// Only include type when $ref is not present (OpenAPI spec requirement)
	if typ != nil {
		fields = append(fields, jsonFieldInfo{"type", typ, omitEmpty})
	}
	fields = append(fields,
		jsonFieldInfo{"title", s.Title, omitEmpty},
		jsonFieldInfo{"description", s.Description, omitEmpty},
		jsonFieldInfo{"$ref", s.Ref, omitEmpty},
		jsonFieldInfo{"format", s.Format, omitEmpty},
		jsonFieldInfo{"contentMediaType", contentMediaType, omitEmpty},
		jsonFieldInfo{"contentEncoding", s.ContentEncoding, omitEmpty},
		jsonFieldInfo{"default", s.Default, omitNil},
		jsonFieldInfo{"examples", s.Examples, omitEmpty},
		jsonFieldInfo{"items", s.Items, omitEmpty},
		jsonFieldInfo{"additionalProperties", s.AdditionalProperties, omitNil},
		jsonFieldInfo{"properties", props, omitEmpty},
		jsonFieldInfo{"const", s.Const, omitNil},
		jsonFieldInfo{"enum", s.Enum, omitEmpty},
		jsonFieldInfo{"minimum", s.Minimum, omitEmpty},
		jsonFieldInfo{"exclusiveMinimum", s.ExclusiveMinimum, omitEmpty},
		jsonFieldInfo{"maximum", s.Maximum, omitEmpty},
		jsonFieldInfo{"exclusiveMaximum", s.ExclusiveMaximum, omitEmpty},
		jsonFieldInfo{"multipleOf", s.MultipleOf, omitEmpty},
		jsonFieldInfo{"minLength", s.MinLength, omitEmpty},
		jsonFieldInfo{"maxLength", s.MaxLength, omitEmpty},
		jsonFieldInfo{"pattern", s.Pattern, omitEmpty},
		jsonFieldInfo{"patternDescription", s.PatternDescription, omitEmpty},
		jsonFieldInfo{"minItems", s.MinItems, omitEmpty},
		jsonFieldInfo{"maxItems", s.MaxItems, omitEmpty},
		jsonFieldInfo{"uniqueItems", s.UniqueItems, omitNil},
		jsonFieldInfo{"required", s.Required, omitEmpty},
		jsonFieldInfo{"dependentRequired", s.DependentRequired, omitEmpty},
		jsonFieldInfo{"minProperties", s.MinProperties, omitEmpty},
		jsonFieldInfo{"maxProperties", s.MaxProperties, omitEmpty},
		jsonFieldInfo{"readOnly", s.ReadOnly, omitNil},
		jsonFieldInfo{"writeOnly", s.WriteOnly, omitNil},
		jsonFieldInfo{"deprecated", s.Deprecated, omitNil},
		jsonFieldInfo{"oneOf", s.OneOf, omitEmpty},
		jsonFieldInfo{"anyOf", s.AnyOf, omitEmpty},
		jsonFieldInfo{"allOf", s.AllOf, omitEmpty},
		jsonFieldInfo{"not", s.Not, omitEmpty},
		jsonFieldInfo{"discriminator", s.Discriminator, omitEmpty},
	)

	return marshalJSON(fields, s.Extensions)
}

// determineType determines the type representation for OpenAPI 3.1 format.
// Returns nil if type should be omitted, otherwise returns the type value.
func (s *Schema) determineType() any {
	if s.Ref != "" || s.Type == "" {
		return nil
	}
